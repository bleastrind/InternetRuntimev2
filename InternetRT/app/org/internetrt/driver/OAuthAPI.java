...
public class OAuthAPI extends Controller {
	...
	public static Result authorizebyworkflow() {
		try {
			...
			String code = SiteInternetRuntime.getAuthcodeForActionFlow(appid,
					secret, rid);
			return ok("{code:\"" + code + "\"}");
		} catch (Exception e) {
			return Controller.badRequest(e.toString());
		}
	}
	...
	public static Result token() {
		try {
			...
			AccessToken accesstoken = SiteInternetRuntime
					.getAccessTokenByAuthtoken(appID, authtoken, appSecret);
